...
function BookFlight() {
   ...
   const [passengers, setPassengers] = useState([{ name: '', age: '', gender: 'MALE', seatNumber: '', seatClass: '' }]);
   ...
   const addPassenger = () => setPassengers([...passengers, { name: '', age: '', gender: 'MALE', seatNumber: '', seatClass: '' }]);
                  ...
                  {passengers.map((p, idx) => (
                     <Card key={idx} className="border-slate-100 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.05)] bg-white rounded-[2rem] overflow-hidden group transition-all">
                        ...
                        <CardContent className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                           ...
                           <div className="space-y-2">
                              <Label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">
                                 Seat Class
                              </Label>
                              <div className="relative">
                                 <select
                                    className="flex h-12 w-full rounded-xl border border-slate-100 bg-slate-50 px-4 text-sm font-bold text-slate-900 focus:outline-none appearance-none cursor-pointer"
                                    value={p.seatClass}
                                    onChange={event => updatePassenger(idx, 'seatClass', event.target.value)}
                                 >
                                    <option disabled={!p.seatClass} value="">Select Class</option>
