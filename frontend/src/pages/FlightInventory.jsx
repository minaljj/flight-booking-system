...
function FlightInventory() {
   ...
   const addMutation = useMutation({
      ...
      onError: (error) => {
         console.error(error);
         toast.error('Failed to add flight (Unauthorized / Token expired)');
      }
   });
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                     ...
                     <div className="space-y-8">
                        ...
                        <FormGroup label="Scheduled Days">
                           <Select
                              defaultValue=""
                              onValueChange={(value) => setNewFlight({ ...newFlight, scheduledDays: value })}
                              className="h-14 bg-slate-50 border-slate-100 rounded-2xl font-bold text-slate-900">
                     ...
                     <div className="space-y-8">
                        ...
                        <FormGroup label="Ticket Cost">
                           <Input
                              ...
                              onChange={event =>
                                 setNewFlight({ ...newFlight, ticketCost: parseFloat(event.target.value) })
                              } />
                        </FormGroup>
                     </div>
                  </div>
                  <div className="mt-12 pt-10 border-t border-slate-100 flex justify-end">
                     <Button className="bg-blue-600 hover:bg-blue-700 w-full md:w-auto h-16 px-16 text-lg font-black text-white rounded-2xl shadow-2xl shadow-blue-500/20 active:scale-95 transition-all"
                        onClick={() => {
                           ...
                           if (!newFlight.ticketCost || newFlight.ticketCost < 1) {
                              toast.error("Ticket cost must be greater than 0");
                              return;
                           }
