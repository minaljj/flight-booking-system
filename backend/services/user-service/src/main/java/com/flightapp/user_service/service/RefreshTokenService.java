...
public class RefreshTokenService {
    ...
    public Optional<RefreshToken> findByToken(String token) {
        ...
        return userRepository.findById(userId).map(user -> {
            ...
            rt.setToken(token);
            rt.setUser(user);
            rt.setExpiryDate(Instant.now().plusMillis(60000)); 
            return rt;
        });
    }
    ...
    public RefreshToken createRefreshToken(Long userId) {
        ...
        redisTemplate.opsForValue().set(REFRESH_TOKEN_PREFIX + token, userId.toString(), refreshTokenDurationMs, TimeUnit.MILLISECONDS);
